...
符号 $P,N,T$ 取自几何数据节点，属性当前值为 $C=(R,G,B,A)$。接口参数包括采样次数 $N_s$、尺度 $L$、差分步长缩放 $k$、表面偏移 $o$、采样锥大小 $w$。
...
半径为 $R$ 的球面上，弧矢满足 $s\approx h^{2}/(2R)$，代入后 $\kappa\approx 1/R$，因此 $G$ 通道以 1/米 为单位。该式在 $u=2h$ 处取得极大值 $1/h$，且在 $h\to 0$ 时不会发散。
...
清零在界面上完成：`群组` 节点与 `材质输出` 节点之间的连线断开后烘焙一趟，属性的四个通道即被写成 (0,0,0,1)，连线随后恢复。另一种做法是在物体数据属性里删除该颜色属性、再新建一个同名属性，新属性的 R、G、B 通道为 0。
...
完成后属性 `厚度` 的三个通道依次为 $R$（厚度，米）、 $G$（曲率，1/米）与 $B/10^{-6}$（已完成的趟数）。

## 示例

龙网格为 124,943 顶点与 249,881 三角面，导入时缩放至 0.01 倍，最大边长 2.02 米。
